  ...
  return (
    <div className="admin-screen">
      ...
      <section className="document-intake">
        ...
        <label>컨테이너<input defaultValue="CONT-260823-014" /></label>
      ...
      <div className="document-workspace">
        <section className="document-register">
          ...
          {DOCUMENTS.map((item) => <button className={item[0] === selected[0] ? 'document-row document-grid is-selected' : 'document-row document-grid'} key={item[0]} onClick={() => setSelected(item)} type="button">{item.map((value, index) => <span className={index === 0 ? 'mono' : ''} key={`${item[0]}-${index}`}>{value}</span>)}</button>)}
        </section>
        <aside className="document-inspector">
          ...
          <button type="button">원문·추출 결과 열기</button>
        </aside>
      </div>
    </div>
  )
